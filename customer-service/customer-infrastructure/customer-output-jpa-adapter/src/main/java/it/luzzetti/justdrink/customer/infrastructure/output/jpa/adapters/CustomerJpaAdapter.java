...
public class CustomerJpaAdapter implements SaveCustomerPort, CustomerIdGeneratorPort, AddressIdGeneratorPort, FindCustomerByIdPort {
    ...
    @Override
    public AddressId generateAddressIdentifier() {
        return AddressId.from(UUID.randomUUID());
    }

    @Override
    public Customer findCustomerById(CustomerId customerId) {
        CustomerJpaEntity theCustomerJpaEntity = customerJpaRepository.findById(customerId.id()).orElseGet(null);
        return customerJpaMapper.toDomain(theCustomerJpaEntity);
    }
}
